  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 bg-gradient-to-br from-[#1e293b] to-[#0f172a] p-8 rounded-3xl border border-white/5 relative overflow-hidden shadow-2xl ring-1 ring-inset ring-white/10">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8 h-full">
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4 text-emerald-500 font-bold text-sm">
                ...
                <span className="text-[#8b919e] font-medium">Expected this month: ₹32.0L</span>
              </div>
            </div>
            <div className="w-full md:w-32 h-32 flex items-center justify-center">
              {/* Simplistic Donut Chart Illusion */}
              <div className="w-24 h-24 rounded-full border-[10px] border-white/5 border-t-emerald-500 border-r-emerald-500/50 flex items-center justify-center relative">
                 <span className="text-[10px] font-bold text-[#8b919e]">75% PAID</span>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-[#171b27] p-8 rounded-3xl border border-white/5 flex flex-col justify-between">
           <div className="space-y-1">
             <p className="text-[10px] font-bold text-[#8b919e] uppercase tracking-widest">Outstanding Fees</p>
           ...
           <div className="space-y-2 mt-6">
             ...
             <button 
               ...
               className="w-full py-3 bg-white/5 hover:bg-white/10 text-[#8b919e] text-[10px] font-bold uppercase tracking-widest rounded-xl transition-all border border-white/5 flex items-center justify-center gap-2"
             >
      ...
      {/* AI Revenue Forecast */}
      <section className="bg-[#1b1f2b] p-6 rounded-3xl border border-emerald-500/20 relative overflow-hidden group">
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
                <button 
                  ...
                  className="p-2 bg-white/5 hover:bg-white/10 rounded-xl text-[#8b919e] border border-white/5"
                >
                    <Download size={18} />
                </button>
        ...
        <div className="bg-[#171b27] rounded-3xl border border-white/5 overflow-hidden">
            <div className="overflow-x-auto">
                <table className="w-full text-left">
                    <thead>
                        <tr className="border-b border-white/5 bg-white/2 pb-4">
                            <th className="p-5 text-[10px] font-bold text-[#8b919e] uppercase tracking-widest">Transaction ID</th>
                            <th className="p-5 text-[10px] font-bold text-[#8b919e] uppercase tracking-widest">Name / Section</th>
                            <th className="p-5 text-[10px] font-bold text-[#8b919e] uppercase tracking-widest">Type</th>
                            <th className="p-5 text-[10px] font-bold text-[#8b919e] uppercase tracking-widest">Amount</th>
                            <th className="p-5 text-[10px] font-bold text-[#8b919e] uppercase tracking-widest text-right">Status</th>
                        </tr>
                    </thead>
                        ...
                        {transactions.map((txn, idx) => (
                            <tr key={idx} onClick={() => window.dispatchEvent(new CustomEvent('show-modal', { detail: { title: 'Transaction Details', content: `View complete details and generate receipt for ${txn.id}.` }}))} className="hover:bg-white/5 transition-colors group cursor-pointer">
                                ...
                                <td className="p-5">
                                    ...
                                    <span className="text-[10px] text-[#8b919e] mt-0.5">{txn.date}</span>
                                </td>
                                <td className="p-5 text-xs text-[#dfe2f2]/80">{txn.type}</td>
                                <td className="p-5 text-xs font-bold text-white">{txn.amount}</td>
